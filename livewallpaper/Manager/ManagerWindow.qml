...
FloatingWindow {
    ...
    // The Debian-safe root owns the IPC target while this window is lazy-loaded.
    ...
    // handle.
    //
    // BUGFIX -- raise()/requestActivate() (as previously called here)
    // are QtQuick.Window's own Window type methods, NOT part of
    // Quickshell's FloatingWindow/QsWindow API -- FloatingWindow is a
    // separate WindowInterface-derived type that only exposes
    // startSystemMove()/startSystemResize() of its own (see
    // https://quickshell.org/docs/types/Quickshell/FloatingWindow/ and
    // WindowInterface in Quickshell's source). Calling raise() there
    // always threw "TypeError: ... raise ... is not a function" (visible
    // in the shell's logs), which aborted the function immediately --
    // so in practice only the `visible = true` line below was ever
    // taking effect, and requestActivate() was never even reached.
    //
    // Since Quickshell exposes no direct "bring an already-visible
    // window to front" call, toggle visible off/on when already shown:
    // this unmaps and remaps the window, which Hyprland (and most other
    // WMs) treats as a fresh top-level surface and focuses/stacks on
    // top by default -- the same practical outcome, using only
    // documented API. Safe to call whether the window was already
    // visible or hidden.
    ...
    // system tray's `show` below) additionally remaps an already-visible
    // window (hide then show), which is exactly the "reuse the existing
    // window, show it, get it focused" sequence this needs -- so
    // `open`/`toggle` now use it too. `close`/`isOpen` are unchanged.
    IpcHandler {
        enabled: managerWindow.ipcEnabled
    ...
    function handleSettingsSearch(query) {
        ...
        const routes = [
            ...
            { page: "diagnostics", terms: ["diagnostic", "health", "backup", "restore", "export", "import", "usage", "statistics", "cleanup", "smart selection"] },
    ...
    // ── Smart Accent Color: window border (Window Border) ─────────────
    // A thin 1.5px frame wrapping the entire window content, kept distinct
    // from the border/title bar drawn by the window manager (Quickshell
    // FloatingWindow doesn't let the app draw over the WM frame). This
    // visual indicator follows `Theme.accent` -- at runtime the value of
    // SmartAccentService when Smart Accent Color is enabled in Settings,
    // or the static mauve default when disabled -- so there's NO need for
    // a separate `activeAccentColor` property: Theme.accent is already
    // this single read-point for the whole app (see Theme.qml notes).
    // `color: "transparent"` + no MouseArea => doesn't block any mouse
    // interaction with the controls underneath.
    Rectangle {
        anchors.fill: parent
        color: "transparent"
        border.width: 1.5

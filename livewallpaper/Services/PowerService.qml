...
QtObject {
    ...
    property Process checkProc: Process {
        ...
        command: ["bash", "-c",
            ...
            "  [ -f \"$f\" ] || continue; " +
            "  found=1; status=\"$(cat \"$f\" 2>/dev/null)\"; " +
            "  if [ -r \"${f%/status}/capacity\" ]; then cap=\"$(cat \"${f%/status}/capacity\" 2>/dev/null)\"; fi; " +
            "  break; " +
            "done; " +
            "if [ \"$found\" -eq 1 ]; then " +
            "  echo has_battery; echo \"$status\"; echo \"$cap\"; " +
            "else echo no_battery; fi"]
        stdout: StdioCollector {
            onStreamFinished: {
                const lines = text.trim().split("\n").filter(l => l.length > 0);
                service.hasBattery = lines.includes("has_battery");
                const status = service.hasBattery && lines.length > 1 ? lines[1].toLowerCase() : "";
                service.onBattery = status === "discharging";
                service.charging = status === "charging" || status === "full";
                const rawCap = service.hasBattery && lines.length > 2 ? parseInt(lines[2], 10) : -1;
                service.batteryPercent = Number.isFinite(rawCap) ? rawCap : -1;
            }
        }
    }

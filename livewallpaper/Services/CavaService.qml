 ...
 * "random" | "rainbow" | "smart") plus whichever of manualColor/randomColor/
...
QtObject {
    ...
    readonly property string colorMode: {
        ...
        return (m === "random" || m === "rainbow" || m === "smart") ? m : "manual";
    }
    ...
    readonly property color visualizerColor: {
        ...
        // Mode 4 -- Smart (Smart Accent Color feature). Reads the same
        // SmartAccentService.accentColor Theme.accent reads (see that
        // service's header) so Music Dock's bars visually match whatever
        // the rest of the app is currently tinted with -- no separate
        // Cava/FIFO/cava.conf change needed, since this only repaints a
        // QML color binding, exactly like manual/random/rainbow above.
        if (colorMode === "smart") return SmartAccentService.accentColor;
        return manualColor;
    }
    ...
    readonly property color pcVisualizerColor: {
        ...
        if (pcColorMode === "smart") return SmartAccentService.accentColor;

 ...
 *     anywhere, when "auto") -- polled every 2s while something is
 *     running/being applied, every 10s when provably idle (see pollTimer
 *     below).
...
QtObject {
    ...
    // Runtime performance controller override. This never overwrites the
    // user's saved FPS preference; it only changes the next local wallpaper
    // launch until the performance controller clears it.
    property string performanceFpsOverride: ""
    readonly property string appliedFps: performanceFpsOverride.length > 0
        ? performanceFpsOverride
        : selectedFps
    ...
    readonly property var streamQualityLevels: ["auto", "144", "240", "360", "480", "720", "1080", "1440", "2160"]
    ...
    function apply(path) {
        ...
        service._stopDispatchProc(applyProc);
        service._stopDispatchProc(navigationProc);
        service._dropQueuedApplyActions();
        ...
        service._stopDispatchProc(webProc);
        _clearStreamState();
        _clearWebState();
        playMode = "wallpapers";
        service.performanceFpsOverride = "";
        // Per-wallpaper performance profile (Services/WallpaperProfileService.qml):
        // a profile pinned to this exact video wins; failing that, a
        // profile pinned to the target monitor; failing that, the
        // user's own selectedResolution/selectedFps as before. See
        // that service's header comment for why this only covers
        // resolution/fps and not GPU decode mode.
        const _profile = WallpaperProfileService.effectiveFor(
            path, selectedMonitor, selectedFps, selectedResolution);
    ...
    // ── Performance Engine FPS override (Services/PerformanceService.qml) ──
    // apply()'s "start fresh" behavior (stop streaming/web, clear all
    // state, launch at the user's own selectedFps) is wrong for this
    // caller: PerformanceService is capping an ALREADY-PLAYING local
    // wallpaper's runtime FPS, not responding to a user selection, so
    // playMode/currentPath must not move and performanceFpsOverride
    // must survive the relaunch instead of being reset to "" like
    // apply() does. Only ever restarts the SAME local wallpaper that
    // was already showing, at the new capped FPS.
    function reapplyWithPerformanceFps(fps, reason) {
        if (service.playMode !== "wallpapers" || !service.currentPath) return;
        const numeric = Math.round(Number(fps));
        if (!isFinite(numeric) || numeric <= 0) return;
        service.performanceFpsOverride = String(numeric);
        service._stopDispatchProc(applyProc);
        service._stopDispatchProc(navigationProc);
        // FPS here is intentionally NOT profile-aware: this numeric
        // value already IS the safety/thermal/adaptive/battery cap
        // PerformanceService just computed, and that must keep
        // outranking a per-wallpaper FPS pin. Resolution isn't part of
        // that decision at all, so it still follows the wallpaper/
        // monitor profile (falling back to the user's own selection).
        const _profile = WallpaperProfileService.effectiveFor(
            service.currentPath, selectedMonitor, selectedFps, selectedResolution);
        service._dispatch(applyProc, "apply_wallpaper.sh", [
            service.currentPath,
            _profile.resolution,
            appliedFps
    ...
    // Inverse of reapplyWithPerformanceFps() above: drops the active
    // runtime cap and restores the wallpaper to the user's own
    // selectedFps. No-op when no override is active, so
    // PerformanceService can call this unconditionally whenever
    // conditions no longer call for a cap, without checking state first.
    function clearPerformanceFpsOverride(reason) {
        if (service.performanceFpsOverride.length === 0) return;
        service.performanceFpsOverride = "";
        if (service.playMode !== "wallpapers" || !service.currentPath) return;
        service._stopDispatchProc(applyProc);
        service._stopDispatchProc(navigationProc);
        // No cap is active any more (performanceFpsOverride was just
        // cleared above, so appliedFps === selectedFps here) -- restore
        // to the wallpaper/monitor profile's own baseline rather than
        // the bare global selection, so a per-wallpaper FPS pin comes
        // back once the safety override that was temporarily beating it
        // goes away.
        const _profile = WallpaperProfileService.effectiveFor(
            service.currentPath, selectedMonitor, selectedFps, selectedResolution);
        service._dispatch(applyProc, "apply_wallpaper.sh", [
    ...
    function reapplyForGpuChange() {
        ...
        if (service.performanceFpsOverride.length > 0) {
            service.reapplyWithPerformanceFps(service.performanceFpsOverride);
        } else {
    ...
    // FIX (Next/Previous/Random mixing MP4 and GIF): GifModeContent.qml
    // and WallpapersModeContent.qml/ActionBar.qml/PlaybackControls.qml all
    // share the exact same TransportButtons -> PlaybackService.next()/
    // previous()/random() wiring (see TransportButtons.qml's own comment),
    // and playMode stays "wallpapers" for BOTH a local video and a local
    // GIF since they go through the same mpvpaper backend. Previously
    // _navigationList() always returned WallpaperService.wallpapers (the
    // MP4/video database -- wallpaper_list.sh deliberately excludes
    // .gif/.apng/.webp, see that script), so pressing Next/Previous/Random
    // while a GIF was playing on the GIF page silently jumped to an MP4
    // instead of the next GIF, and vice versa.
    //
    // Fix: pick the navigation pool from whichever type is ACTUALLY
    // playing right now (service.currentPath's extension), not from a
    // single hardcoded source. GIFs are resolved from
    // GifPlaylistService.gifs (already the single reactive scan of the
    // GIF folder used elsewhere for the auto-playlist), reshaped into the
    // same {path, name} shape _applyNavigationTarget()/next()/previous()/
    // random() already expect from WallpaperService.wallpapers -- no
    // change to any of that downstream logic. When nothing is playing yet
    // (currentPath empty), this falls back to the MP4 list, matching the
    // original default behavior.
    function _isGifPath(path) {
        return typeof path === "string" && /\.gif$/i.test(path);
    }

    function _navigationKind() {
        return _isGifPath(service.currentPath) ? "gif" : "wallpaper";
    }

    function _navigationList() {
        if (_navigationKind() === "gif") {
            return (GifPlaylistService.gifs || []).map(p => {
                const slash = p.lastIndexOf("/");
                return { path: p, name: slash >= 0 ? p.substring(slash + 1) : p };
            });
        }
        return WallpaperService.wallpapers || [];
    }

    // Kicks off a rescan of whichever pool next()/previous()/random() just
    // found empty, and tells the user, mirroring the pre-existing
    // WallpaperService.refresh() fallback below for the MP4 case.
    function _refreshNavigationSource(kind) {
        if (kind === "gif") {
            GifPlaylistService.rescan();
            NotifyService.info("Refreshing GIF wallpapers…");
        } else {
    ...
    function _applyNavigationTarget(target) {
        ...
        // Same per-wallpaper/monitor profile resolution as apply()
        // above; appliedFps (not bare selectedFps) is still the
        // fallback so an active performance-engine cap keeps applying
        // across Next/Previous/Random the same as it always has.
        const _profile = WallpaperProfileService.effectiveFor(
            target.path, selectedMonitor, appliedFps, selectedResolution);
    ...
    function next() {
        const kind = _navigationKind();
        const list = _navigationList();
        if (list.length === 0) {
            _refreshNavigationSource(kind);
            return;
        }
        let index = -1;
        ...
        _applyNavigationTarget(list[targetIndex]);
    }
    ...
    // shorter name for callers like a Slider's onMoved.
    ...
    // Deliberately terminates a still-running shared dispatch Process
    // (applyProc/navigationProc/webProc) because a newer request has
    // superseded it -- e.g. clicking Apply/Next/Previous/Random again
    // before the previous dispatch finished. Setting `.running = false`
    // on an active Process sends it SIGTERM, so its onExited still fires
    // with a non-zero/signal exit code and (since the process had no
    // chance to write anything) empty stderr. Marking `_selfCancelled`
    // here lets that onExited tell "we killed this on purpose" apart
    // from "the command genuinely failed" and skip _dispatchExited --
    // mirrors the generation-tracking streamProc already uses for the
    // exact same kind of intentional supersede-and-kill.
    function _stopDispatchProc(proc) {
        if (proc.running) {
            proc._selfCancelled = true;
            proc.running = false;
        }
    }
    ...
    property FileView currentView: FileView {
        ...
        // printErrors: false since onLoadFailed below already handles this
        // known/expected case; no need for Quickshell to log it as a warning.
        printErrors: false
    ...
    property FileView statusView: FileView {
        ...
        watchChanges: true
        // printErrors: false -- see currentView above; onLoadFailed below
        // already treats a missing status file as the normal/expected case.
        printErrors: false
        onFileChanged: reload()
        onLoaded: {
            try {
    ...
    property Process applyProc: Process {
        id: applyProc
        property bool _selfCancelled: false
        stderr: StdioCollector { id: applyErr }
        onExited: (code, status) => {
            if (applyProc._selfCancelled) {
                applyProc._selfCancelled = false;
                return;
            }
            service._dispatchExited(code, applyErr.text);
        }
    }
    property Process navigationProc: Process {
        ...
        onExited: (code, status) => {
            if (navigationProc._selfCancelled) {
                // A newer Apply/Next/Previous/Random superseded this
                // dispatch and we killed it ourselves -- not a failure,
                // and currentPath will be reconciled by whichever newer
                // dispatch's own currentView.reload() (or its own exit
                // handling) supersedes this one.
                navigationProc._selfCancelled = false;
                return;
            }
            if (code !== 0) {
    ...
    property Process webProc: Process {
        ...
        onExited: (code, status) => {
            if (webProc._selfCancelled) {
                webProc._selfCancelled = false;
                return;
            }
            if (code !== 0) {
    ...
    property Timer pollTimer: Timer {
        // PERFORMANCE FIX: adaptive interval. 2s while something could
        // plausibly be alive (running, or a dispatch is in flight) so an
        // mpvpaper crash is still noticed promptly; 10s when provably
        // idle -- there is nothing to watch over, and the slow idle tick
        // only exists to notice an mpvpaper started OUTSIDE this app.
        // Cuts idle background process spawns 5x (1800/hr -> 360/hr).
        // Changing `interval` on a running QML Timer restarts it with the
        // new interval, so a Start/Apply (busy -> true) snaps polling back
        // to 2s immediately; every successful-apply/stop path already
        // fires pollRunning.running = true itself (see statusView.onLoaded
        // and _repathForMonitor), so `running` never waits a full idle tick.
        interval: service.running || service.busy ? 2000 : 10000
        running: true
        repeat: true
        triggeredOnStart: true
    ...
    function smartStopAllWallpapers() {
        ...
        // dispatch while the trigger condition persists (e.g. the 10s

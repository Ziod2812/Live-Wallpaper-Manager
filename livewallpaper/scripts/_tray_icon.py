...
def ipc(target, method_name):
    ...
    cmd = (
        ...
        '|| ("$HOME/.config/quickshell/livewallpaper/scripts/launch_quickshell.sh" '
        "-c livewallpaper -n >/dev/null 2>&1 & sleep 1.5; "
        "quickshell -c livewallpaper ipc call {t} {m} 2>/dev/null)"
    ).format(t=t, m=m)
    try:
        subprocess.Popen(
...
def _find_livewallpaper_pid():
    """Return the PID of the running Live Wallpaper Manager Quickshell instance."""
    try:
        proc = subprocess.run(
            ["ps", "-eo", "pid=,args="],
        ...
        for line in proc.stdout.splitlines():
            ...
            if pid == os.getpid():
                continue
            if re.search(r"(^|[ /])quickshell[ ]+-c[ ]+livewallpaper([ ]|$)", args):

...
LW_APPLY_STATUS_FILE="$LW_CACHE_DIR/apply_status"
LW_LOG_DIR="$LW_CACHE_DIR/logs"
LW_STATE_HOME="${XDG_STATE_HOME:-$HOME/.local/state}"
LW_REPORT_LOG_DIR="$LW_STATE_HOME/LiveWallpaperLogs"
...
LW_TRANSITION_REQUEST_FILE="$LW_CACHE_DIR/transition-request.json"

# ---------------------------------------------------------------------------
# AWWW TRANSITIONS
# ---------------------------------------------------------------------------
# AWWW is the real transition backend for animated-GIF wallpapers only.
# MP4/video wallpapers continue to use the existing persistent mpvpaper path.
# The UI intentionally exposes only enable/disable + duration; the transition
# effect is fixed to AWWW's selected transition mode so there is no fake QML
# screenshot animation competing with the compositor wallpaper backend.

# Reads transition_enabled/transition_duration/transition_type from
# settings.json with a SINGLE jq call and caches the result for the
# lifetime of this shell process. A full AWWW apply (enabled-check +
# type + duration, sometimes read twice for the log line too) used to
# spawn jq up to 6x per wallpaper switch; now it's at most 1.
# Safe to cache: every caller here is a short-lived, one-shot script
# invocation (_apply_worker.sh etc.), never a long-running loop that
# would need to observe a mid-run settings change.
lw_transition_settings_load() {
    [ -n "${_LW_TRANSITION_LOADED:-}" ] && return 0
    _LW_TRANSITION_ENABLED='false'
    _LW_TRANSITION_DURATION='1.0'
    _LW_TRANSITION_TYPE='fade'
    if [ -s "$LW_SETTINGS_FILE" ] && command -v jq >/dev/null 2>&1; then
        local line
        # NOTE: do NOT use `.transition_enabled // true` here -- jq's //
        # operator treats `false` as an empty/falsy value just like `null`,
        # so it would silently replace an explicit `false` with `true` and
        # make the "Enable transitions" toggle a no-op (always reads as
        # enabled regardless of the real saved setting). Only fall back to
        # the default when the key is genuinely absent (null).
        line="$(jq -r '
            [ (if .transition_enabled == null then false else .transition_enabled end),
              (.transition_duration // 1.0),
              (.transition_type // "fade") ] | @tsv
        ' "$LW_SETTINGS_FILE" 2>/dev/null)"
        if [ -n "$line" ]; then
            IFS=$'\t' read -r _LW_TRANSITION_ENABLED _LW_TRANSITION_DURATION _LW_TRANSITION_TYPE <<< "$line"
        fi
    fi
    _LW_TRANSITION_DURATION="$(awk -v d="$_LW_TRANSITION_DURATION" \
        'BEGIN { if (d+0 < 0.15 || d+0 > 3.0) d=1.0; printf "%.3f", d+0 }')"
    case "$_LW_TRANSITION_TYPE" in
        none|simple|fade|left|right|top|bottom|wipe|wave|grow|center|any|outer|random) ;;
        *) _LW_TRANSITION_TYPE='fade' ;;
    esac
    _LW_TRANSITION_LOADED=1
}

lw_transition_enabled() {
    lw_transition_settings_load
    [ "$_LW_TRANSITION_ENABLED" = "true" ]
}

lw_transition_duration() {
    lw_transition_settings_load
    printf '%s' "$_LW_TRANSITION_DURATION"
}
...
lw_awww_available() {
    lw_awww_bin >/dev/null 2>&1 && lw_awww_daemon_bin >/dev/null 2>&1
}

# Fills the array named by $1 with the AWWW transition flags for the given
# transition type/duration. Shared by the still-image apply path and the
# video-frame transition path so the two flag sets can't drift apart when
# one of them gets tweaked and the other is forgotten.
lw_awww_transition_args() {
    local -n _lw_trans_out="$1"
    local transition="$2" duration="$3"
    case "$transition" in
        none)   _lw_trans_out=(--transition-type none --transition-step 255) ;;
        simple) _lw_trans_out=(--transition-type simple --transition-step 2) ;;
        *)      _lw_trans_out=(--transition-type "$transition" \
                    --transition-duration "$duration" \
                    --transition-fps 60 --transition-step 90) ;;
    esac
}

# Fills the array named by $1 with the --outputs flag for $2, or leaves it
# empty for "" / "auto" (apply to all outputs).
lw_awww_output_args() {
    local -n _lw_out_out="$1"
    local monitor="$2"
    if [ -n "$monitor" ] && [ "$monitor" != "auto" ]; then
        _lw_out_out=(--outputs "$monitor")
    else
        _lw_out_out=()
    fi
}

lw_ensure_awww_daemon() {
    local awww daemon log_file i
    awww="$(lw_awww_bin 2>/dev/null)" || return 1
    daemon="$(lw_awww_daemon_bin 2>/dev/null)" || return 1

    # First prefer the daemon's own IPC health check. This also works when a
    # user launched it outside this application.
    if "$awww" query >/dev/null 2>&1; then
        return 0
    fi

    log_file="$LW_LOG_DIR/awww-daemon.log"
    mkdir -p "$LW_LOG_DIR"
    # Avoid spawning duplicates. A successful query above is the authoritative
    # check; pgrep is only a conservative fallback while startup settles.
    if ! pgrep -x awww-daemon >/dev/null 2>&1; then
        setsid "$daemon" >>"$log_file" 2>&1 < /dev/null &
    fi
    ...
    lw_log_warn "AWWW daemon did not become ready (see $log_file)"
    return 1
}

lw_apply_awww_wallpaper() {
    # lw_apply_awww_wallpaper <path> <monitor>
    local path="$1" monitor="$2" awww duration transition
    [ -f "$path" ] || return 1
    # Call the loader directly (not via "$(...)") so the single jq read it
    # does lands in THIS shell, not a throwaway subshell -- then read the
    # cached globals directly instead of re-invoking the getters, which
    # would each fork their own subshell and still cost a jq call apiece.
    lw_transition_settings_load
    duration="$_LW_TRANSITION_DURATION"
    transition="$_LW_TRANSITION_TYPE"
    [ "$_LW_TRANSITION_ENABLED" = "true" ] || transition="none"
    awww="$(lw_awww_bin 2>/dev/null)" || return 1
    lw_ensure_awww_daemon || return 1

    local -a out_args=() trans_args=()
    lw_awww_output_args out_args "$monitor"
    lw_awww_transition_args trans_args "$transition" "$duration"

    "$awww" img "${out_args[@]}" "${trans_args[@]}" "$path"
}

lw_clear_awww_output() {
    # lw_clear_awww_output <monitor>
    local monitor="$1" awww
    awww="$(lw_awww_bin 2>/dev/null)" || return 0
    "$awww" query >/dev/null 2>&1 || return 0
    if [ -n "$monitor" ] && [ "$monitor" != "auto" ]; then
        "$awww" clear --outputs "$monitor" 000000 >/dev/null 2>&1 || true
    else
...
# ---------------------------------------------------------------------------
# LIVE VIDEO TRANSITION
# ---------------------------------------------------------------------------
# For MP4/video wallpaper switches, AWWW is used as a temporary compositor
# transition layer fed by a frame extracted directly from the NEW video.
# This avoids capturing the desktop/UI and avoids starting a second MPV.
# After the transition window, the existing mpvpaper instance is switched in
# place over MPV IPC and the temporary AWWW layer is cleared.
# ---------------------------------------------------------------------------

lw_apply_awww_video_transition() {
    # lw_apply_awww_video_transition <video> <monitor>
    # Returns 0 when the AWWW frame transition was launched successfully.
    local video="$1" monitor="$2" awww duration transition tmp frame_ext
    [ -f "$video" ] || return 1

    # Check the transition setting BEFORE touching the daemon. Otherwise
    # lw_ensure_awww_daemon below would spawn awww-daemon on every video
    # change even while the user has "Enable transitions" turned off,
    # leaving the daemon running despite the toggle being off.
    lw_transition_settings_load
    [ "$_LW_TRANSITION_ENABLED" = "true" ] || return 1
    transition="$_LW_TRANSITION_TYPE"
    [ "$transition" != "none" ] || return 1
    duration="$_LW_TRANSITION_DURATION"
    ...
    # Extract from the video itself, never from the screen/output. This keeps
    # panels, bars and other desktop layers out of the transition frame.
    tmp="$LW_CACHE_DIR/transition-video"
    mkdir -p "$tmp"
    frame_ext="$tmp/frame-$$-$RANDOM.png"
    if ! ffmpeg -hide_banner -loglevel error -y -ss 0 -i "$video" \
            -frames:v 1 -vf format=rgba "$frame_ext"; then
        rm -f "$frame_ext"
        return 1
    fi
    ...
    local rc=0
    "$awww" img "${out_args[@]}" "${trans_args[@]}" "$frame_ext" >/dev/null 2>&1 || rc=$?
...
#
# MUST use printf, not echo: `echo "$1" | tr -c ... '_'` silently appends
# one extra spurious "_" to EVERY result, forever, on EVERY call. echo
# adds a trailing newline; `tr -c 'A-Za-z0-9_.-' '_'` treats that
# newline as "not in the allowed set" and rewrites it into a literal "_"
# instead of it being stripped -- $(...) only strips a trailing newline,
# and by then tr had already turned it into an underscore, so there's
# nothing left to strip. Every call this way permanently manufactures
# one more character than it was given. Chained together across a
# session (anywhere the sanitized name gets fed back in as input to a
# later sanitize call, e.g. after being read back from an existing
# state directory) that compounds into ever-longer garbage directory
# names ("eDP-1_", "eDP-1__", "eDP-1___", ...) that stop matching what
# anything else on the system expects. printf '%s' emits no trailing
# newline, so tr has nothing spurious to rewrite, and the function is
# now a true no-op when re-applied to its own output.
...
#      data.
#
# TWO TIERS -- each caller picks the variant that matches the file it is
# writing:
#   - DURABLE   (lw_atomic_commit / lw_write_text_atomic): full guarantees
#     1-4. Used for genuine user data (settings.json, wallpapers.json,
#     history.json, collections.json, ...) where the last write must
#     survive a power loss.
#   - ATOMIC-ONLY (lw_atomic_commit_fast / lw_write_text_atomic_fast):
#     guarantees 1 + 3 only (same-dir temp + single atomic rename), with
#     NO fsync. Used for regenerable runtime cache/state under
#     $LW_CACHE_DIR (per-monitor current/last/resolution/fps/pid/
#     launch_sig, worker_pid, apply_status, ...). What readers like
#     Quickshell's FileView watchChanges actually depend on is never
#     seeing a half-written file -- not the bytes surviving an unexpected
#     power cut, which means nothing for state that is regenerated or
#     auto-detected on the next start anyway. Skipping the two `sync -f`
#     subprocesses per write is the single biggest hot-path win in this
#     file: one wallpaper switch writes up to a dozen small state files,
#     and each fsync pair costs a coreutils process + filesystem sync
#     syscall on both sides of the rename.
# ---------------------------------------------------------------------------
...
lw_atomic_tmp_for() {
    local target="$1" dir
    dir="$(dirname "$target")"
    # Skip the mkdir -p fork when the directory already exists (the common
    # case -- every script calls lw_ensure_dirs / lw_monitor_state_dir
    # before writing, so this is pure overhead on most writes).
...
# lw_atomic_commit_fast <tmp_file> <target_file>
# ATOMIC-ONLY tier of lw_atomic_commit (same-dir rename, no fsync) -- see
# the two-tier note in the CRASH-SAFE ATOMIC WRITES header. Callers must
# only use this for regenerable cache/state under $LW_CACHE_DIR, never for
# user data.
lw_atomic_commit_fast() {
    local tmp="$1" target="$2"
    if [ ! -f "$tmp" ]; then
        return 1
    fi
...
# lw_write_text_atomic_fast <file> <value>
# ATOMIC-ONLY tier of lw_write_text_atomic: keeps the same-dir temp +
# atomic rename that READERS (Quickshell's FileView watchChanges, the
# other scripts) depend on, but skips the two `sync -f` subprocesses.
# A wallpaper switch writes ~10 regenerable state files on its critical
# path; going durable on all of them costs ~20 coreutils forks + sync
# syscalls a switch. Reserved for $LW_CACHE_DIR runtime state where a
# lost write after a power cut is meaningless (it is re-derived on next
# start), never for settings/database/history JSON -- see the header block
# above for the full tier definition.
lw_write_text_atomic_fast() {
    local file="$1" value="$2" tmp
    tmp="$(lw_atomic_tmp_for "$file")" || return 1
    if ! printf '%s\n' "$value" > "$tmp"; then
        rm -f "$tmp"
    ...
    lw_atomic_commit_fast "$tmp" "$file"
}
...
lw_write_apply_status() {
    ...
    # Build the JSON payload ONCE with a single jq call (previously the
    # primary write and the legacy mirror each ran their own `jq -n` +
    # `date +%s`, doubling every status update's subprocess cost on the
    # wallpaper-switch hot path). $EPOCHSECONDS is a bash builtin -- no
    # `date` fork needed.
    local json
    if ! json="$(jq -n \
        --arg state "$state" \
        ...
        '{state: $state, video: $video, message: $message, timestamp: $ts}')"; then
        return 1
    fi

    # apply_status is regenerable runtime state (one wallpaper switch simply
    # overwrites it), so the ATOMIC-ONLY tier (rename, no fsync) is the
    # right fit -- the FileView reader only depends on never reading a
    # half-written file, and the next apply/stop clobbers it anyway.
    local tmp
    tmp="$(lw_atomic_tmp_for "$status_file")" || return 1
    printf '%s\n' "$json" > "$tmp" || { rm -f "$tmp"; return 1; }
    lw_atomic_commit_fast "$tmp" "$status_file"

    # Legacy global mirror (same convention as LW_CURRENT_FILE/LW_LAST_FILE/
    # LW_RESOLUTION_FILE/LW_FPS_FILE in _apply_worker.sh, and the manual
    # dual-write stop_wallpaper.sh already does for this exact file).
    #
    # PlaybackService.qml's statusView/currentView default to (and, on a
    # single-monitor system, NEVER leave) the "auto" monitor -- i.e. this
    # exact top-level path -- while every caller here normally omits an
    # explicit monitor and gets one auto-detected internally (lw_detect_monitor),
    # which resolves to a PER-MONITOR path instead. Without this mirror, that
    # mismatch means the QML side never sees the status update at all: no
    # "Applying..."/success/error toast, no matter whether the underlying
    # apply actually succeeded or failed (e.g. the Wayland-required check in
    # lw_wayland_required_error) -- it just looks like clicking Apply does
    # nothing.
    if [ "$status_file" != "$LW_APPLY_STATUS_FILE" ]; then
        tmp="$(lw_atomic_tmp_for "$LW_APPLY_STATUS_FILE")" || return 0
        printf '%s\n' "$json" > "$tmp" || { rm -f "$tmp"; return 0; }
        lw_atomic_commit_fast "$tmp" "$LW_APPLY_STATUS_FILE"
    fi
}

lw_ensure_dirs() {
    mkdir -p "$LW_WALLPAPER_DIR" "$LW_THUMB_DIR" "$LW_DATA_DIR" "$LW_LOG_DIR" "$LW_REPORT_LOG_DIR"
...
# Recovers WAYLAND_DISPLAY (and XDG_RUNTIME_DIR) when they're missing from
# the CURRENT process's environment, even though a real Wayland session is
# actually running -- and forces QT_QPA_PLATFORM=wayland once we know one is.
#
# WHY THIS EXISTS:
# A process started by clicking an app-launcher icon does not always
# inherit the same environment as a process started from a terminal.
# Depending on the desktop environment / session manager, .desktop Exec=
# commands can be spawned via dbus activation or a systemd --user unit
# that was started before (or without) the compositor's session
# environment being imported into it -- so WAYLAND_DISPLAY (and
# sometimes other vars) can simply be ABSENT for an icon-launched
# process, even on a machine where a plain terminal has it set correctly
# and everything works fine run by hand. Previously launch_quickshell.sh
# only forced QT_QPA_PLATFORM=wayland when WAYLAND_DISPLAY WAS present --
# so on a setup hitting this gap, the guard never fired, Qt's wayland
# platform plugin had no socket name to connect to, and quickshell
# failed to start with no visible output at all (icon click -> nothing,
# silently) instead of ever reaching the xcb-specific crash this project
# already defended against.
#
# This recovers WAYLAND_DISPLAY by finding the actual compositor socket
# on disk (the same thing WAYLAND_DISPLAY would normally just name)
# instead of trusting the environment to have propagated it correctly.
# A no-op (leaves everything untouched) if WAYLAND_DISPLAY is already
# set, or if no wayland-* socket can be found (a genuine X11-only
# session, or no session at all).
# ---------------------------------------------------------------------------
lw_ensure_wayland_env() {
    # Fast path: already have it, nothing to recover.
    if [ -n "${WAYLAND_DISPLAY:-}" ]; then
        export QT_QPA_PLATFORM=wayland
        return 0
    fi

    # XDG_RUNTIME_DIR is where the wayland-N socket (and its wayland-N.lock
    # sibling) lives. It's supposed to always be set for a graphical user
    # session, but the same launcher-environment gap that can drop
    # WAYLAND_DISPLAY can drop this too -- fall back to the standard
    # /run/user/<uid> location the spec mandates if it's missing.
    local runtime_dir="${XDG_RUNTIME_DIR:-/run/user/$(id -u)}"
    [ -d "$runtime_dir" ] || return 0

    # Prefer a socket that actually has a live compositor listening (its
    # .lock file present and held), then fall back to the first
    # plausible wayland-N socket name found. Sorted so "wayland-0" wins
    # over "wayland-1" etc. when several exist (e.g. a leftover socket
    # from a previous crashed session).
    local sock found=""
    for sock in "$runtime_dir"/wayland-*.lock; do
        [ -e "$sock" ] || continue
        local name
        name="$(basename "$sock" .lock)"
        [ -S "$runtime_dir/$name" ] && { found="$name"; break; }
    done
    if [ -z "$found" ]; then
    ...
    if [ -n "$found" ]; then
        ...
        export QT_QPA_PLATFORM=wayland
        lw_log_info "lw_ensure_wayland_env: WAYLAND_DISPLAY was missing from this process's environment -- recovered '$found' from $runtime_dir and forced QT_QPA_PLATFORM=wayland"
    fi
}
...
lw_log() {
    local level="$1"; shift
    local msg="$*"
    [ -d "$LW_LOG_DIR" ] || mkdir -p "$LW_LOG_DIR"
    local line
...
lw_log_watcher() {
    [ -d "$LW_LOG_DIR" ] || mkdir -p "$LW_LOG_DIR"
    printf '[%s] %s\n' "$(date '+%Y-%m-%d %H:%M:%S')" "$*" >> "$LW_WATCHER_LOG_FILE" 2>/dev/null || true
    lw_log_info "$*"
}
...
lw_detect_monitor() {
    ...
    elif command -v kscreen-doctor >/dev/null 2>&1 && command -v jq >/dev/null 2>&1; then
        # KDE Plasma (KWin) has no hyprctl and no per-output "focused"
        # concept the way Hyprland does -- kscreen-doctor -j is KDE's own
        # output-listing tool. Prefer the primary output; fall back to the
        # first enabled one so a single-monitor KDE box still resolves to
        # its real output name instead of the hardcoded "eDP-1" guess
        # below (which only happened to be right when that guess matched
        # reality).
        local detected
        detected="$(kscreen-doctor -j 2>/dev/null | jq -r \
            '(.outputs // [])
             | (map(select(.primary==true)) + map(select(.enabled==true)) + .)
...
# lw_wayland_session_ok -> 0 (true) on a Wayland session, 1 (false)
# otherwise (X11, or nothing detectable). mpvpaper is built entirely on
# the Wayland wlr-layer-shell protocol -- it cannot connect to an X11
# display at all, so launching it under Plasma X11 (or any other X11
# session) fails outright rather than just glitching. Callers use this
# to fail fast with a clear, actionable status message instead of
# letting mpvpaper die with an opaque "could not connect to a wayland
# compositor"-style error buried in its log.
lw_wayland_session_ok() {
    [ -n "${WAYLAND_DISPLAY:-}" ] && return 0
    [ "${XDG_SESSION_TYPE:-}" = "wayland" ] && return 0
    return 1
}

# lw_wayland_required_error <status_file> <source> -- writes the shared
# "not on Wayland" error to apply_status and returns 1. Callers do:
#   lw_wayland_session_ok || { lw_wayland_required_error "$status_file" "$SOURCE"; exit 1; }
lw_wayland_required_error() {
    local status_file="$1" source="$2"
    lw_write_apply_status "$status_file" "error" "$source" \
        "mpvpaper needs a Wayland session and this one is X11 (or undetected). Video wallpaper/streaming can't run here -- install the KDE Plasma wallpaper plugin instead (scripts/install_kde_plasma_wallpaper.sh), which plays video natively without mpvpaper and works on both Plasma X11 and Wayland."
    return 1
}
...
#
# BUGFIX (Quit/Stop leaves the app it launched running): every tracked
# backend here (mpvpaper, and the chromium/firefox kiosk browser launched
# by _web_worker.sh's "local" HTML path) is started with `setsid`, which
# makes it the leader of a brand-new session AND process group (its pgid
# equals its own pid). mpvpaper never forks children, so signaling it
# alone was always enough -- but a kiosk browser does: Chromium/Chrome
# spawn separate zygote/GPU/renderer child processes that inherit that
# same process group and do NOT self-terminate just because their parent
# died. Signaling only the tracked pid (the old behavior) could kill the
# visible browser window while leaving those children running in the
# background. Signaling the whole process group (negative pid) reaches
# the tracked process AND every child that inherited its group, while
# still touching only this one setsid'd group -- mpvpaper's own group has
# exactly one member, so its behavior is unchanged. Falls back to a
# plain single-pid signal if the process-group form is ever rejected
# (e.g. pid is somehow not a group leader).
# ---------------------------------------------------------------------------
lw_kill_pid_wait() {
    local pid="$1"
    [ -n "$pid" ] || return 0
...
EOF
    ...
    if [ -n "$monitor" ] && [ "$monitor" != "auto" ]; then
        ...
        for stray_pid in $(pgrep -x mpvpaper 2>/dev/null); do
            if tr '\0' '\n' < "/proc/$stray_pid/cmdline" 2>/dev/null | grep -qx -- "$monitor"; then
...
# ---------------------------------------------------------------------------
# lw_resolve_stream_playback_url <url> <quality>
# ---------------------------------------------------------------------------
# YouTube-only. Resolves <url> to a directly-playable URL (or an mpv
# edl:// combining a separate video+audio pair) using yt-dlp's explicit non-Android YouTube
# extractor clients, instead of letting mpv's bundled ytdl_hook.lua resolve
# it internally. Two independent problems this works around:
#
#   1. mpv's ytdl_hook.lua can end up selecting YouTube's ANDROID_VR client,
#      whose https GVS URLs may return HTTP 403 without the required PO token.
#      Force web_embedded + web_safari instead.
#   2. Even where (1) doesn't apply, overriding the client from mpv's side
#      requires ytdl_hook.lua's "raw_options" script-opt, which several
#      distro-packaged mpv/ytdl_hook.lua builds (observed on Fedora, built
#      against system mpv-libs) simply don't have -- passing
#      --script-opts=ytdl_hook-raw_options=... is silently ignored
#      ("unknown key raw_options"), so there was no way to fix (1) from
#      mpv's side at all on those builds.
#
# Resolving here sidesteps both: it's our own yt-dlp invocation, so it
# always honors --extractor-args regardless of what ytdl_hook.lua on the
# system supports.
#
# Prints the resolved URL/edl:// string on success, or nothing on failure
# (caller falls back to handing mpv the original URL and letting its
# built-in ytdl_hook try, same as before this existed -- never a hard
# failure just because pre-resolution didn't pan out).
#
# Trade-off: the resolved googlevideo.com URL expires in a few hours.
# The current streaming worker treats this as a one-shot media URL and does
# not run a separate refresh worker.
# ---------------------------------------------------------------------------
lw_resolve_stream_playback_url() {
    local url="$1" quality="${2:-auto}"
    command -v yt-dlp >/dev/null 2>&1 || { echo ""; return 1; }

    # Only YouTube needs this -- Twitch/Vimeo/etc aren't hitting the
    # ANDROID_VR 403 issue, so leave them to mpv's own ytdl_hook untouched.
    echo "$url" | grep -qiE 'youtu\.?be' || { echo ""; return 1; }

    local height_cap fmt
    height_cap="$(lw_resolution_to_height "$quality" 2>/dev/null)"
    [ -z "$height_cap" ] && case "$quality" in
        [0-9]*) height_cap="$quality" ;;
    esac
    if [ -n "$height_cap" ]; then
        # Explicit quality is strict: select only a video stream at the
        # requested height, paired with the best available audio.  Do not
        # fall back to an arbitrary higher/lower video stream.
        fmt="bestvideo[height=${height_cap}]+bestaudio/b[height=${height_cap}]"
    else
        fmt="bestvideo+bestaudio/best"
    fi

    local urls line_count video_url audio_url
    # --js-runtimes node: yt-dlp only auto-enables "deno" by default for
    # solving YouTube's JS signature/n-parameter challenge -- an installed
    # `node` binary is otherwise silently ignored ("JS runtimes: none" in
    # -v output even with node on PATH), so this must be explicit. Without
    # a JS runtime actually in use, yt-dlp falls back to storyboard-only
    # formats and `-f "$fmt"` fails with "Requested format is not
    # available". See the `nodejs` dependency in install.sh's DEP_TABLE.
    # --remote-components ejs:github: harmless/no-op for the official
    # standalone binary (bundles the EJS solver script already) but keeps
    # this working if yt-dlp was ever installed a different way (pip
    # without the [default] extra, a distro package, etc).
    urls="$(yt-dlp --no-warnings \
                    --extractor-args "youtube:player_client=web_embedded,web_safari" \
                    --js-runtimes node \
                    --remote-components "ejs:github" \
                    -f "$fmt" -g "$url" 2>/dev/null)"
    line_count="$(printf '%s\n' "$urls" | grep -c .)"

    case "$line_count" in
        2)
            # bestvideo+bestaudio -> yt-dlp prints the video URL first,
            # audio second. Combine as an mpv edl:// with two streams,
            # same construction mpv's own ytdl_hook.lua uses internally.
            video_url="$(printf '%s\n' "$urls" | sed -n '1p')"
            audio_url="$(printf '%s\n' "$urls" | sed -n '2p')"
            [ -z "$video_url" ] || [ -z "$audio_url" ] && { echo ""; return 1; }
            printf 'edl://!no_chapters;%%%d%%%s;!new_stream;!no_chapters;%%%d%%%s' \
                "${#video_url}" "$video_url" "${#audio_url}" "$audio_url"
            return 0
            ;;
        1)
            # A single pre-merged (progressive) format was selected.
            printf '%s' "$urls"
            return 0
            ;;
        *)
...
lw_launch_mpvpaper() {
    ...
    for i in $(seq 1 80); do
        ...
        if [ -n "$new_pid" ]; then
            # Compositor-connect failure time varies (well under a second
            # to a few seconds), so poll for up to 4s and only trust this
            # as a real success if still alive at the end, breaking out
            # early if it dies sooner. Without this, a compositor-connect
            # failure gets reported as "playing" while nothing is on screen.
            still_alive=true
            ...
            lw_write_text_atomic_fast "$(lw_monitor_state_file "$monitor" "pid")" "$new_pid"
...
lw_mpv_write_launch_sig() {
    ...
    lw_write_text_atomic_fast "$(lw_monitor_state_file "$monitor" "launch_sig")" \
...
lw_mpv_try_reuse() {
    ...
    for attempt in $(seq 1 "$max_attempts"); do
        ...
        # WALLPAPER-SWITCH FIX (black-flash on Next/Previous/Random): a plain
        # `loadfile ... replace` on the same mpv process still leaves a brief
        # blank gap between the old demuxer tearing down and the new file's
        # first frame being decoded -- this is inherent to mpv's own reload,
        # not a kill/relaunch (this whole function never kills a live PID).
        # _mpv_ipc.py already implements real in-player transitions (fade via
        # lavfi, pan/zoom for slide/wipe/wave/zoom -- see
        # _transition_before_load/_transition_after_load) that mask exactly
        # this gap, but they used to be wired ONLY into the AWWW/GIF path,
        # never into this normal MPV reuse call, which always passed the
        # hardcoded "none"/"0" below -- so video wallpapers never got any
        # cover for the reload gap regardless of the user's transition
        # setting. Honor the SAME existing "Enable transitions" setting
        # (lw_transition_enabled/lw_transition_type/lw_transition_duration,
        # already used for AWWW GIFs) here too. Default is unchanged: when
        # transitions are disabled (the app default), this still sends
        # "none"/"0" exactly as before -- zero behavior change unless the
        # user has opted in via the existing Settings toggle.
        local transition_arg="none" duration_arg="0"
        if lw_transition_enabled; then
            transition_arg="$(lw_transition_type)"
            duration_arg="$(lw_transition_duration)"
        fi
        ipc_result="$(python3 "$py" "$sock" apply-wallpaper "$video" "$vf_combined" "$transition_arg" "$duration_arg" 2>/dev/null)"
...
# ---------------------------------------------------------------------------
# SMART ACCENT COLOR — fire-and-forget dispatch of apply_smart_color.py
# after a wallpaper/GIF/live-wallpaper has ACTUALLY been applied
# successfully (called from _apply_worker.sh's two success paths). See
# scripts/apply_smart_color.py's own header for the full extraction +
# race-condition design; this helper only handles DISPATCH:
#
#   - Cheap early-out via jq when the feature is off, so a normal wallpaper
#     switch never pays the cost of starting a Python interpreter for
#     nothing (this is on the hot path of every single wallpaper change).
#   - setsid + disown, exactly like every other background dispatch in
#     this file (_apply_worker.sh itself, start_cava.sh, etc.) -- the
#     calling worker script returns immediately; nothing here ever blocks
#     a wallpaper switch, and nothing here is left as an orphaned
#     foreground job attached to this shell's job control.
#   - Never fails loud: if python3 is missing, or the settings read fails,
#     this silently no-ops -- a missing/broken Smart Accent Color pipeline
#     must never turn into a failed wallpaper apply.
lw_trigger_smart_accent() {
    local media_path="$1"
    local monitor="${2:-}"
    [ -z "$media_path" ] && return 0
    [ -f "$media_path" ] || return 0

    if [ -s "$LW_SETTINGS_FILE" ] && command -v jq >/dev/null 2>&1; then
        local enabled
        enabled="$(jq -r '.smart_accent_enabled // false' "$LW_SETTINGS_FILE" 2>/dev/null)"
        [ "$enabled" = "true" ] || return 0
    else
        # Cannot confirm the setting either way (jq missing or settings.json
        # not yet initialized) -- default to NOT dispatching, matching the
        # feature's default-off posture rather than guessing.
        return 0
    fi

    command -v python3 >/dev/null 2>&1 || return 0

    setsid python3 "$SCRIPT_DIR/apply_smart_color.py" "$media_path" \

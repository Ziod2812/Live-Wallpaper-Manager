...
# A short 1-second debounce is used (wallpaper applies run immediately via
# live-mpv IPC reuse, but each stream URL needs a fresh yt-dlp resolve +
# mpvpaper relaunch, so a quick window lets an immediately-superseded
# request be cancelled instead of paid for).
#
...
lw_write_text_atomic_fast "$(lw_monitor_state_file "$MONITOR" "worker_pid")" "$worker_pid"

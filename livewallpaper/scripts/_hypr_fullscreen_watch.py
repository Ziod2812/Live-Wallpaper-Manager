...
def main():
    ...
    # Defensive ceiling only -- Hyprland's socket2 protocol is always
    # newline-delimited and individual event lines are a few hundred
    # bytes at most, so this never trips in normal operation. It exists
    # purely so a misbehaving/foreign process on the socket path (or a
    # future Hyprland protocol change that drops the delimiter) can't
    # grow `buf` unbounded in this long-running daemon -- once the
    # undelimited backlog exceeds the ceiling, drop it and resync on the
    # next newline instead of accumulating memory forever.
    MAX_BUF_BYTES = 1 << 20  # 1 MiB
    buf = b""
    try:
        while True:
            ...
            if len(buf) > MAX_BUF_BYTES:
                # No newline arrived across 1 MiB of data -- this is not
                # a valid Hyprland event stream. Discard the backlog and
                # resync cleanly on whatever comes next rather than
                # growing further or trying to salvage a partial line.
                buf = b""
                continue
            while b"\n" in buf:

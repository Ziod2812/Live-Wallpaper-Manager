...
#
#            SMART ACCENT COLOR: the Chromium/Chrome launch below adds
#            --allow-file-access-from-files so a local wallpaper's own
#            HTML can `fetch("file:///tmp/livewallpaper_color.current")`
#            at runtime (see assets/web/smart-accent-bridge.js) --
#            without this flag Chromium blocks file:// pages from
#            fetching other file:// resources by default. This script
#            does NOT itself extract a color for web wallpapers (there is
#            no local media file to sample a thumbnail from for either
#            "url" or "local" sources) -- the bridge script simply reads
#            whatever accent color the last applied image/GIF/video wrote.
#
...
if [ "$TYPE" = "url" ]; then
    if command -v mpvpaper >/dev/null 2>&1; then
        ...
        if [ "$(lw_mpv_try_reuse "$SOURCE" "$MONITOR" "original" "original" "mode=web")" = "true" ]; then
            lw_write_text_atomic_fast "$(lw_monitor_state_file "$MONITOR" "current")" "web:$SOURCE"
...
if [ "$TYPE" = "url" ]; then
    ...
    # Plasma X11) it can't connect to anything and would otherwise just
    # die with an opaque error. Fail fast with a clear, actionable
    # message instead. The "local" (browser kiosk) path below doesn't
    # need this -- a fullscreen browser window works under X11 too.
    ...
    # Same settle-check as _stream_worker.sh -- compositor-connect failure
    # time varies (well under a second to a few seconds), so poll for up
    # to 4s and only trust this as success if still alive at the end,
    # breaking out early if it dies sooner.
    if [ -n "$new_pid" ]; then
        still_alive=true
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    if kill -0 "$browser_pid" 2>/dev/null; then
        lw_write_text_atomic_fast "$pid_file" "$browser_pid"
        lw_write_text_atomic_fast "$(lw_monitor_state_file "$MONITOR" "current")" "web-local:$SOURCE"

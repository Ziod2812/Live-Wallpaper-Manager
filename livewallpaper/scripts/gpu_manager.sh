...
# ---------------------------------------------------------------------------
_lw_gpu_stats_json() {
    local gpus_json items=() pci vendor carddir
    gpus_json="$(_lw_gpu_list_json)"
    ...
    # pci/vendor come straight off this same jq call as @tsv instead of
    # two more jq forks per GPU (the previous version fed raw JSON
    # objects through the loop and re-parsed each one with
    # `echo "$g" | jq -r '.pci'` / `.vendor` separately). Both fields are
    # always non-empty by construction in _lw_gpu_list_json (pci is
    # `continue`-guarded, vendor always defaults to "unknown"), so plain
    # tab-separated @tsv is safe here -- no risk of bash's IFS-whitespace
    # collapsing silently dropping an empty column.
    while IFS=$'\t' read -r pci vendor; do
        [ -n "$pci" ] || continue
        ...
        case "$vendor" in
            ...
            amd)
                ...
                ;;
            intel)
                # Intel iGPU: prefer kernel gpu_busy_percent when available.
                # Fallback to intel_gpu_top (JSON) when accessible.
                if carddir="$(_lw_gpu_card_dir_for_pci "$pci")"; then
                    if [ -r "$carddir/device/gpu_busy_percent" ]; then
                        v="$(cat "$carddir/device/gpu_busy_percent" 2>/dev/null)"
                        [ -n "$v" ] && util="$v"
                    elif command -v intel_gpu_top >/dev/null 2>&1; then
                        v="$(timeout 1 intel_gpu_top -J 2>/dev/null | jq -r '.engines["Render/3D"].busy // .engines.Render.busy // empty' | head -n1)"
                        [ -n "$v" ] && util="${v%.*}"
                    fi
                    if v="$(_lw_gpu_hwmon_temp_c "$carddir")"; then temp="$v"; fi
                fi
    ...
    done < <(echo "$gpus_json" | jq -r '.[] | [.pci, .vendor] | @tsv')

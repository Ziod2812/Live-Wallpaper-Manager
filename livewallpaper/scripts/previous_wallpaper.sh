#!/usr/bin/env bash
#
# previous_wallpaper.sh [monitor]
# ------------------------
# Switches to the previous wallpaper in the database (in display order),
# wrapping to the end of the list if at the start. Keeps that monitor's
...
MONITOR="${1:-}"
target="$(lw_neighbor_wallpaper "previous" "$MONITOR")"

if [ -z "$target" ]; then
    lw_log_warn "previous_wallpaper.sh: database empty, nothing to switch to"

...
PanelWindow {
    ...
    // In the standalone Debian-safe shell, the root owns the IPC target.
    // Embedded users can keep the panel-local IPC by leaving this enabled.
    ...
    Component.onCompleted: { depCheckProc.running = true; applyFrostedGlass(); }
    ...
    // -------------------- REAL FROSTED GLASS (COMPOSITOR BLUR) --------
    // The wallpaper is NOT rendered in this QML scene -- mpvpaper owns it
    // on its own wlr-layer-shell Background surface -- so a FastBlur/
    // MultiEffect here could never sample the desktop. The only layer
    // that can frost the animated wallpaper behind these translucent
    // panels is the compositor, so we drive Hyprland's real multi-pass
    // blur from here: scripts/frosted_glass.sh enables `decoration:blur:*`
    // and adds a `blur` + `ignorealpha` layerrule for every namespace this
    // app owns (this panel, Music Dock, Peaclock+Cava Dock, trigger dock).
    // `blurPct` mirrors Theme.uiBgOpacity (the sliders's stored index), so
    // the compositor radius stays linked to the Background blur slider:
    // dragging the slider DOWN (blur % -> maximum) writes ui_bg_opacity
    // toward its 0.15 floor, which lands here as a SMALL blurPct -> the
    // script's `(100 - pct)` mapping yields size 12 -- MAXIMUM blur, and
    // the wallpaper collapses into an unreadable frosted smudge behind
    // the (now micaAlpha-densified) UI. Dragging it UP eases blurPct
    // toward 100 -> size 3. Re-applied (debounced) whenever the slider
    // moves; cleaned up by ApplicationService.exit().
    // Non-Hyprland compositors are skipped safely -- the script itself
    // guards `command -v hyprctl`.
    readonly property int blurPct: Math.round(Theme.uiBgOpacity * 100)

    function applyFrostedGlass() { frostDebounce.start(); }

    // Re-apply live whenever the slider moves: blurPct re-evaluates on
    // every Theme.uiBgOpacity change (the Manager app writes
    // ui_bg_opacity to settings.json; this panel's SettingsService
    // FileView watchChanges picks it up), and onBlurPctChanged re-runs
    // the same debounced compositor apply so the real blur radius
    // follows the slider in real time -- not just once at startup.
    // frostProc.command re-binds to the new blurPct, so the debounce
    // below always carries the LATEST radius when it fires.
    onBlurPctChanged: applyFrostedGlass()

    Timer {
        id: frostDebounce
        interval: 300
        onTriggered: frostProc.running = true
    }
    Process {
        id: frostProc
        command: ["bash", "-c", Paths.script("frosted_glass.sh") + " apply " + blurPct]
    }

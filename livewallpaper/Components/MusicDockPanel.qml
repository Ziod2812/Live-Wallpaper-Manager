...
Rectangle {
    ...
    // ── Drag anywhere -- see Panels/MusicDockOverlay.qml's "Drag-anywhere
    // (freeform position)" section. draggable just gates the drag surface;
    // freePosition reflects whether a custom dragged spot is currently
    // active (vs one of the Bottom/Center/Top presets above).
    ...
    // How far the dock moves per pixel of cursor travel while dragging,
    // as a percentage -- 100 = 1:1 (the original, unscaled feel), lower
    // values track more slowly/precisely, higher values cover more
    // ground per flick. Read by MusicDockOverlay.qml's
    // dockDragArea._applyLatest() as a plain multiplier on dx/dy. Not in
    // settings.sh's DEFAULT_SETTINGS/SettingsService's default settings
    // object -- same guarded-fallback-only convention every other
    // music_dock_* key on this page already follows (see header).
    readonly property int dragSensitivityVal: {
        const v = parseInt(SettingsService.settings.music_dock_drag_sensitivity, 10);
        return (v >= 25 && v <= 400) ? v : 100;
    }
    ...
    ColumnLayout {
        ...
        // Mode 4 -- Smart (Smart Accent Color). No manual/random controls
        // here -- the color is entirely derived from the currently applied
        // wallpaper by apply_smart_color.py (see SmartAccentService.qml).
        // This section only shows a live swatch + a pointer to the global
        // toggle, since turning the FEATURE itself on/off lives in
        // Settings, not here (one master switch, not one per consumer).
        SettingRow {
            label: "Smart accent color"
            visible: CavaService.colorMode === "smart"
            RowLayout {
                spacing: Theme.spacingSm
                Rectangle {
                    width: 22; height: 22; radius: 11
                ...
                Text {
                    text: SmartAccentService.enabled
                        ? "Following the current wallpaper's color"
                        : "Enable \"Smart Accent Color\" in Settings to activate"
                    color: Theme.subtext0
            ...
            RowLayout {
                ...
                Repeater {
                    ...
                    delegate: IconButton {
                        text: modelData.label
                        fontSize: Theme.fontSizeSm
                        active: root.positionVal === modelData.key && !root.freePosition
                        onClicked: {
                            // Picking a preset always wins over a dragged
                            // free position -- see MusicDockOverlay.qml's
                            // "Drag-anywhere" header.
                            SettingsService.set("music_dock_free_position", "false");
                            SettingsService.set("music_dock_position", modelData.key);
                        }
                    }
                }
            }
        ...
        // ── Drag anywhere ────────────────────────────────────────────────
        // Toggle just flips music_dock_draggable, read by
        // MusicDockOverlay.qml's dockDragArea (covers the whole dock --
        // album art, text, blank background, not one specific control).
        // Reset drops back to whichever Bottom/Center/Top preset is
        // selected above -- same effect as double-clicking the dock.
        SettingRow {
            label: "Drag dock anywhere"
            RowLayout {
                spacing: Theme.spacingSm
                ToggleSwitch {
                    checked: root.draggable
                    onToggled: SettingsService.set("music_dock_draggable", !root.draggable)
                }
                IconButton {
                    text: "Reset position"
                    fontSize: Theme.fontSizeSm
                    visible: root.freePosition
                    onClicked: SettingsService.set("music_dock_free_position", "false")
                }
            }
        }
        // How far the dock travels per pixel of cursor movement while
        // being dragged -- see dragSensitivityVal's header above. Only
        // meaningful while dragging is actually enabled.
        SliderRow {
            label: "Drag sensitivity"
            visible: root.draggable
            from: 25; to: 400; stepSize: 5
            value: root.dragSensitivityVal
            suffix: "%"
            onMoved: SettingsService.set("music_dock_drag_sensitivity", Math.round(value))
        }

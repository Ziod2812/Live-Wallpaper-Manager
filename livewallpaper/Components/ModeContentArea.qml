...
Item {
    ...
    readonly property var modeOrder: ["wallpapers", "gif", "streaming", "web"]
    ...
    Component.onCompleted: {
        wallpapersWrap.visible = root.visibleMode === "wallpapers";
        gifWrap.visible = root.visibleMode === "gif";
        streamingWrap.visible = root.visibleMode === "streaming";
        webWrap.visible = root.visibleMode === "web";
    }
    ...
    Item {
        ...
        width: parent.width
        visible: root.visibleMode === "wallpapers"
    ...
    Item {
        id: gifWrap
        anchors.fill: parent
        visible: root.visibleMode === "gif"
        opacity: visible ? 1 : 0

        Loader {
            id: gifLoader
            anchors.fill: parent
            active: root.visibleMode === "gif"
            source: active ? "GifModeContent.qml" : ""
            onLoaded: {
                if (item)
                    item.zenMode = root.zenMode
            }
        }
    }
    ...
    function focusSearch() {
        if (root.visibleMode === "gif" && gifLoader.item)
            gifLoader.item.focusSearch();
        else

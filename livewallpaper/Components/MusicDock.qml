 ...
 * Since v2.9.b the REAL blur behind this layered translucency is provided
 * by the compositor instead -- scripts/frosted_glass.sh enables Hyprland's
 * decoration blur + a `blur` layerrule for this dock's layer namespace --
 * while this QML only keeps the artistic translucent layer on top of it
 * (in-app wallpaper imagery uses QtQuick.Effects MultiEffect, see

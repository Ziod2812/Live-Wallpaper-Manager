...
Rectangle {
    ...
    // ── Drag anywhere -- see Panels/PeaclockCavaDockOverlay.qml's
    // "Drag-anywhere (freeform position)" section. Same shape as Music
    // Dock's identical setting (Components/MusicDockPanel.qml), own
    // "pcdock_draggable"/"pcdock_free_position"/"pcdock_drag_sensitivity"
    // keys -- dragging/tuning this dock never touches Music Dock's own
    // "music_dock_*" drag settings, and vice versa.
    ...
    ColumnLayout {
        ...
        // Mode 4 -- Smart (Smart Accent Color). Same shape as Music
        // Dock's identical section (Components/MusicDockPanel.qml) --
        // read-only swatch + status text, the actual on/off toggle for
        // the feature itself lives in Settings.
        SettingRow {
            label: "Smart accent color"
            visible: CavaService.pcColorMode === "smart"
            RowLayout {
                spacing: Theme.spacingSm
                Rectangle {
                    width: 22; height: 22; radius: 11
                ...
                Repeater {
                    ...
                    delegate: IconButton {
                        ...
                        onClicked: {
                            ...
                            // free position -- see PeaclockCavaDockOverlay.
                            // qml's "Drag-anywhere" header.
                            SettingsService.set("pcdock_free_position", "false");
                            SettingsService.set("pcdock_position", modelData.key);
                        }
                    }
                }
            }
        ...
        // ── Drag anywhere ────────────────────────────────────────────────
        // Toggle just flips pcdock_draggable, read by
        // PeaclockCavaDockOverlay.qml's dockDragArea (covers the whole
        // dock -- clock, date, Cava strip, now-playing text, blank
        // background, not one specific control). Reset drops back to
        // whichever Bottom/Center/Top + Left/Center/Right preset is
        ...
        SettingRow {
            ...
            RowLayout {
                ...
                ToggleSwitch {
                    ...
                    onToggled: SettingsService.set("pcdock_draggable", !root.draggable)
                }
                IconButton {
                    text: "Reset position"
                    fontSize: Theme.fontSizeSm
                    visible: root.freePosition
                    onClicked: SettingsService.set("pcdock_free_position", "false")
                }
            }
        }
        ...
        SliderRow {
            ...
            suffix: "%"
            onMoved: SettingsService.set("pcdock_drag_sensitivity", Math.round(value))
        }

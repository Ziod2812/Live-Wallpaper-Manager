...
The v2.9 codebase retains weather-related backend services for future weather
automation. The current release does **not** expose a dedicated Weather page,
weather-rule editor, or location editor in the main UI.

Backend components include:

- `Services/WeatherService.qml`
- `scripts/weather.sh`
- `scripts/weather_tick.sh`

The backend can evaluate weather conditions using Open-Meteo data, but this
should be considered an internal capability rather than a fully exposed UI
feature in v2.9.
...
The current release focuses on stability, Wayland/Hyprland compatibility,
wallpaper lifecycle reliability, performance controls, and cleanup/hardening.

Key fixes and hardening areas include:
...
The project retains its earlier release history for reference. Older release
notes describe the evolution of performance controls, profiles, history,
playlist functionality, and other features introduced before v2.9.

For the detailed old-vs-v2.9 comparison covering known bugs, fixes, hardening,
